...
<<<<<<< HEAD
		...
		// Calculate the actual date based on selected day using current week logic
...
<<<<<<< HEAD
			...
			formData.append('notes', `${activityType} appointment for ${patientText}. ${notes ? 'Notes: ' + notes : ''}`);
...
<<<<<<< HEAD
				...
				alert('Error creating appointment: ' + (result.message || 'Unknown error'));
			}
		} catch (error) {
...
<<<<<<< HEAD
			...
			if (patientResult.success && patientResult.patients) {
				// Find the patient by name
				const patient = patientResult.patients.find(p => 
			...
			if (!patientId) {
				alert('Patient not found. Please make sure the patient is registered in the system.');
				return;
			}
			...
			formData.append('notes', `${appointmentType} appointment for ${patientName}`);
...
<<<<<<< HEAD
	...
	// Load appointments from database - using the same endpoint as doctor's personal schedule
	async function loadAppointmentsFromDatabase(doctorId) {
		try {
			// Use the same endpoint as doctor's personal schedule to get ALL appointments
			// This ensures appointments like Maria Santos (Sept 8) show up in admin view
			// AND when deleted from database, they disappear from both views
			const response = await fetch(`<?= site_url('scheduling/getDoctorAppointments') ?>/${doctorId}`);
...
<<<<<<< HEAD
					...
					result.appointments.forEach(appointment => {
						...
						doctor.appointments.push({
							...
							date: appointmentDate,
							duration: '1 hour',
							endTime: appointmentTime.toTimeString().slice(0, 5)
						});
					});
...
<<<<<<< HEAD
					...
					monday.setDate(new Date().getDate() - daysSinceMonday);
...
<<<<<<< HEAD
		...
		if (appointment.type.toLowerCase().includes('treatment')) {
			// Treatment = Violet/Purple
			backgroundColor = '#f3e8ff';
			borderColor = '#9333ea';
			textColor = '#7c3aed';
		} else if (appointment.type.toLowerCase().includes('lab') || appointment.type.toLowerCase().includes('review')) {
			// Lab Review = Orange
			backgroundColor = '#fff7ed';
			borderColor = '#ea580c';
			textColor = '#c2410c';
		} else if (appointment.type.toLowerCase().includes('consultation')) {
			// Consultation = Green
...
<<<<<<< HEAD
	...
	window.navigateWeek = function(direction) {
		...
		// Refresh the display with new week
		if (currentDoctorKey) {
			renderDetail(currentDoctorKey);
		}
	};
...
<<<<<<< HEAD
	...
	async function resetSampleAppointments() {
		...
		try {
			...
			const doctorId = currentDoctorKey || Object.keys(doctors)[0];
			if (!doctorId) {
				alert('❌ No doctor selected');
				return;
			}
			
			// Delete all appointments for this doctor from database
			const response = await fetch('<?= site_url('scheduling/deleteAllAppointments') ?>/' + doctorId, {
				method: 'POST',
				headers: {
					'X-Requested-With': 'XMLHttpRequest'
				}
			...
			if (result.success) {
				console.log('✅ All appointments deleted from database');
				
				// Clear appointments from frontend
				if (doctors[doctorId]) {
					doctors[doctorId].appointments = [];
				}
				...
				allCells.forEach(cell => {
					if (cell.querySelector('.appointment-card')) {
						cell.innerHTML = '';
					}
				});
				...
				// Update appointment count display
				if (currentDoctorKey && doctors[currentDoctorKey]) {
				...
				alert('❌ Error deleting appointments: ' + (result.message || 'Unknown error'));
			}
		} catch (error) {
...
>>>>>>> 52c85cbe61791b823437f19869f9d0a43509eb56
	...
	window.resetSampleAppointments = resetSampleAppointments;
	window.checkAppointmentCount = checkAppointmentCount;
	window.refreshAppointmentsFromDatabase = refreshAppointmentsFromDatabase;
